...
def _find_hermes_md(cwd: Path) -> Optional[Path]:
    ...
    # _isfile_or_denied: an unreadable directory (stale cwd after user migration) is "not here", not a crash.
    for directory in [current, *current.parents] if stop_at else [current]:
        found = next((directory / n for n in (".hermes.md", "HERMES.md") if _isfile_or_denied(directory / n)), None)
        if found or directory == stop_at:
            return found
    return None


def _isfile_or_denied(path: Path) -> bool:
    try:
        return path.is_file()
    except OSError:
        return False
...
def build_context_files_prompt(
    ...
    if cwd is None and not allow_install_tree_fallback and _is_install_tree(cwd_path):
        ...
        # before a root -> unprivileged-user migration). Any probe inside it
        # raises PermissionError, so skip project-context discovery entirely.
        logger.warning(
